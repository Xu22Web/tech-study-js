            ...
            if (mainStore.settings[SettingType.REMOTE_PUSH]) {
                ...
                const res = await pushModal({
                    ...
                    content: [
                        ...
                        ...mainStore.tasks.map((task) => getProgressHTML(task.title, task.percent.value)),
                    ],
                    type: 'success',
                }, mainStore.pushToken);
...
(function (ProgressType) {
    ProgressType[ProgressType["LOADING"] = 0] = "LOADING";
    ProgressType[ProgressType["LOADED"] = 1] = "LOADED";
    ProgressType[ProgressType["START"] = 2] = "START";
    ProgressType[ProgressType["PAUSE"] = 3] = "PAUSE";
    ProgressType[ProgressType["FINISH"] = 4] = "FINISH";
 ...
 * @returns
 */
function trigger(target, newVal, oldVal) {
    // target 订阅列表
...
const createReactiveObj = (target, isReadonly, shallow) => {
    return (new Proxy(target, createReactiveHandlers(isReadonly, shallow)));
};
/**
 ...
 * @returns
 */
const watchEffectRef = (refVal, callback) => {
    if (isRef(refVal)) {
        // 影响
        const effect = ref(callback(refVal.value));
        // 监听
        watchEffect(() => (effect.value = callback(refVal.value)));
        return effect;
    }
};
/**
 ...
 * @returns
 */
const watchEffectRefs = (refVal, callback) => {
    if (Array.isArray(refVal)) {
        // 影响
        const effect = ref(callback(refVal.map((v) => unref(v))));
        refVal.forEach((ref) => {
            watch(ref, () => (effect.value = (callback(refVal.map((v) => unref(v))))));
        });
        return effect;
    }
};
...
async function Info({ login }) {
        ...
        if (userInfo) {
            ...
            return createElementNode('div', undefined, {
                ...
                createElementNode('div', undefined, { class: 'egg_userinfo' }, [
                    ...
                    // 昵称
                    createElementNode('div', undefined, { class: 'egg_nick' }, createTextNode(nick)),
                ]),
                // 退出按钮
    ...
    return createElementNode('div', undefined, {
        ...
        // 窗口
        createElementNode('div', undefined, {
            class: watchEffectRef(mainStore.startLogin, () => `egg_login_img_wrap${mainStore.startLogin.value ? ' active' : ''}`),
        }, createElementNode('img', undefined, {
            class: 'egg_login_img',
        })),
    ]);
    ...
    if (login) {
        ...
        return createElementNode('div', undefined, { class: 'egg_scoreinfo' }, [
            createElementNode('div', undefined, {
                ...
                createTextNode('总积分'),
                createElementNode('span', undefined, undefined, createTextNode(mainStore.totalScore)),
            ]),
            createElementNode('div', undefined, {
                ...
                createElementNode('button', undefined, {
                    ...
                    createTextNode('当天分数'),
                    // 当天分数
                    createElementNode('span', undefined, undefined, createTextNode(mainStore.todayScore)),
                    // icon
                    ...
                    createElementNode('div', undefined, {
                        class: watchEffectRef(scoreShow, () => `egg_score_details${scoreShow.value ? '' : ' hide'}`),
                    }, [
 ...
 * @returns
 */
function TimeInput({ hour, minute, }) {
    // 小时
    ...
    watchEffect(() => {
        ...
        if (list) {
            const index = hours.findIndex((h) => !!(hour.value && h.label.includes(String(hour.value))));
            hours.forEach((h, i) => {
                if (index + 1) {
    ...
    watchEffect(() => {
        ...
        if (list) {
            const index = minutes.findIndex((min) => !!(minute.value && min.label.includes(String(minute.value))));
    ...
    return createElementNode('div', undefined, { class: 'egg_time_input' }, [
        createElementNode('div', undefined, { class: 'egg_hour_wrap' }, [
            ...
            createElementNode('div', undefined, {
                class: watchEffectRef(focusHour, () => `egg_list${focusHour.value ? '' : ' hide'}`),
            }, hours.map((v) => createElementNode('div', undefined, {
                class: watchEffectRef(v.active, () => `egg_time${v.active.value ? ' focus' : ''}`),
                onclick: () => {
        ...
        createElementNode('div', undefined, { class: 'egg_minute_wrap' }, [
            ...
            createElementNode('div', undefined, {
                class: watchEffectRef(focusMinute, () => `egg_list${focusMinute.value ? '' : ' hide'}`),
            }, minutes.map((v) => createElementNode('div', undefined, {
                class: watchEffectRef(v.active, () => `egg_time${v.active.value ? ' focus' : ''}`),
                onclick: () => {
 ...
 * @returns
 */
function SchedulePanel({ scheduleList, show, }) {
    // token
    let token = '';
    // 小时
    let hour = ref(-1);
    ...
    let minute = ref(-1);
    return createElementNode('div', undefined, {
        class: watchEffectRef(show, () => `egg_schedule${show.value ? ' active' : ''}`),
    }, [
 ...
 * @returns
 */
function TaskItem({ title, tip, checked, onChange, percent, }) {
    return createElementNode('div', undefined, { class: 'egg_setting_item' }, [
        createElementNode('div', undefined, { class: 'egg_label_wrap' }, [
...
function Panel({ login }) {
    ...
    return createElementNode('div', undefined, {
        ...
        class: watchEffectRef(panelShow, () => `egg_panel${panelShow.value ? ' hide' : ''}`),
    }, [
        createElementNode('div', undefined, {
            class: 'egg_user_item',
        }, Info({ login })),
        ...
        createElementNode('div', undefined, {
            ...
            createElementNode('button', undefined, {
                class: watchEffectRefs([mainStore.frameShow, mainStore.frameExist], () => `egg_frame_show_btn${!mainStore.frameExist.value || mainStore.frameShow.value
                    ? ' hide'
                    : ''}`),
                type: 'button',
                onclick: () => {
            ...
            createElementNode('button', undefined, {
                class: mainStore.settings[SettingType.REMOTE_PUSH] ||
                    mainStore.settings[SettingType.SCHEDULE_RUN]
                    ? watchEffectRef(scheduleShow, () => `egg_setting_push_btn${scheduleShow.value ? ' active' : ''}`)
        ...
        login
            ? createElementNode('div', undefined, { class: 'egg_study_item' }, createElementNode('button', undefined, {
                class: watchEffectRef(mainStore.progress, () => `egg_study_btn${mainStore.progress.value === ProgressType.LOADING ||
                    mainStore.progress.value === ProgressType.START
                    ? ' loading'
                    : mainStore.progress.value === ProgressType.FINISH
                        ? ' disabled'
                        : ''}`),
                type: 'button',
                disabled: watchEffectRef(mainStore.progress, () => mainStore.progress.value === ProgressType.LOADING ||
                    mainStore.progress.value === ProgressType.FINISH),
            }, createTextNode(watchEffectRef(mainStore.progress, () => `${mainStore.progress.value === ProgressType.LOADING
                ? '等待中'
                : mainStore.progress.value === ProgressType.LOADED
                    ? '开始学习'
                    : mainStore.progress.value === ProgressType.START
...
function Frame({ show }) {
    ...
    // 容器
    return createElementNode('div', undefined, {
        class: watchEffectRef(show, () => `egg_frame_wrap${show.value ? '' : ' hide'}`),
    }, [
        // 遮罩
        createElementNode('div', undefined, { class: 'egg_frame_mask' }),
        // 窗口内容
        createElementNode('div', undefined, {
            class: watchEffectRef(max, () => `egg_frame_content_wrap ${max.value ? ' max' : ''}`),
        }, [
            // 窗口控制
...
function createTip(text, delay = 2, callback) {
    ...
    // 文本
    const span = createElementNode('span', undefined, {
        class: 'egg_text',
    ...
    const tipInfo = createElementNode('div', undefined, {
        class: watchEffectRef(show, () => `egg_tip${show.value ? ' active' : ''}`),
    }, [span, countdown]);
    // 操作
    const operate = {
        async destroy() {
...
function ExamBtn() {
    return createElementNode('button', undefined, {
        class: watchEffectRef(mainStore.examPause, () => `egg_exam_btn${mainStore.examPause.value ? ' manual' : ''}`),
        type: 'button',
        onclick: () => {
            mainStore.examPause.value = !mainStore.examPause.value;
        },
    }, createTextNode(watchEffectRef(mainStore.examPause, () => `${mainStore.examPause.value ? '开启自动答题' : '关闭自动答题'}`)));
}
