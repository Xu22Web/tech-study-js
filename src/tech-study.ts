...
const API_CONFIG = {
  ...
  // 每周答题列表
  weeklyList:
    'https://pc-proxy-api.xuexi.cn/api/exam/service/practice/pc/weekly/more?pageSize=50&pageNo={pageNo}',
  // 专项练习列表
  paperList:
    'https://pc-proxy-api.xuexi.cn/api/exam/service/paper/pc/list?pageSize=50&pageNo={pageNo}',
  // 文本服务器保存答案
...
function getNextButton() {
  return new Promise(function (resolve) {
    let nextInterVal = setInterval(() => {
      // 答题按钮
      const nextAll = Array.from($$('.ant-btn')).filter(
  ...
  while (true) {
    ...
    if (
      ...
      if (nextButton.textContent === '下一题' && $$('.answer')[0]) {
        ...
        await saveAnswer(key, answer);
        if (
          URL_CONFIG.examWeekly.indexOf(url) !== -1 ||
